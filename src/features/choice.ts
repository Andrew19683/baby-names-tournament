...
const btnChooseNode = document.getElementById(
  "choose-btn",
...
const textareaEdit = document.getElementById(
  "edit-textarea",
...
let winnerId: number = 0; // для определения победившего имени
let loserId: number = 0; // для определения проигравшего имени
let currentEditingName: string; // для определения какому имени редактируем описание
...
function getRandomMatch() {
  ...
  if (
    isSameDay(
      currentDate,
      new Date(localStorage.getItem("lastPlayedDate") ?? "null"),
    )
  ) {
...
function setMatchResult() {
  ...
  const matches = JSON.parse(localStorage.getItem("matches") ?? "null");
  const currentMatch =
    matches.boys.find((match: Match) => match.status === "currentMatch") ||
    matches.girls.find((match: Match) => match.status === "currentMatch");
...
function saveDescription(name: string, description: string) {
  const descriptions =
    JSON.parse(localStorage.getItem("descriptions") ?? "null") || {};
