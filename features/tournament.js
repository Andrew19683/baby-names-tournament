...
export function startTournament(names, gender) {
...
export function generateMatches(names, gender) {
    ...
    while (participatsForRound >= 2) {
        ({ matchId, nextLoserMatchId: loserMatchId } = generateUpperRound(matches, participatsForRound, matchId, round, loserMatchId, names, gender));
    ...
    while (participatsForRound >= 2) {
        matchId = generateLowerRound(matches, participatsForRound, matchId, round, gender);
        if (round % 2 === 0) {
            participatsForRound /= 2;
        }
        round++;
...
function generateUpperRound(matches, participants, matchId, round, nextLoserMatchId, names, gender) {
    ...
    for (let i = 0; i < participants / 2; i++) {
        ...
        if (!finalMatch) {
            ...
            if (round % 2 === 1) {
                ...
                match.loserGoesId = Math.trunc(nextLoserMatchId + participants / 2 - i * 2 - 1);
                nextLoserMatchId += 1;
            }
        }
        else {
...
export function findMatchById(matches, gender, id) {
    return matches[gender].find((match) => match.id == id);
}
export function resolveMatch(matches, matchId, gender, winnerId, loserId) {
    const match = matches[gender].find((match) => match.id === matchId);
    ...
    const winnerGoesMatch = findMatchById(matches, match.gender, match.winnerGoesId);
    ...
    if (match.grid === "upper") {
        const loserGoesMatch = findMatchById(matches, match.gender, match.loserGoesId);
...
export function playByeMatches(matches) {
    ...
    allMatches.forEach((match) => {
        if (match.status === "readyToPlay" &&
            (match.player1.isBye === true || match.player2.isBye === true)) {
            if (match.player1.isBye === true) {
                resolveMatch(matches, match.id, match.gender, match.player2.id, match.player1.id);
            }
            else {
                resolveMatch(matches, match.id, match.gender, match.player1.id, match.player2.id);
            }
        }
    });
}
function getStrandings(matches, gender) {
    ...
    const namesCount = JSON.parse(localStorage.getItem("names") ?? "null").filter((name) => name.gender === gender.slice(0, -1) && !name.isBye).length;
    ...
    if (matches[gender].find((match) => match.id === 1000).status === "finished") {
        standings[1] = [
    ...
    for (let i = maxRound; i >= 1; i--) {
        ...
        const endPlace = startPlace + step - 1 > namesCount ? namesCount : startPlace + step - 1; // посчитаем, до какого места рпоигравшие его делят
...
function isGrandFinalFinished(gender) {
    ...
    return (matches[gender].find((match) => match.id === 1000).status ===
        "finished");
}
// рендер
function renderRoundsNumbers(matches, querySelector, gender, grid) {
    const rounds = matches[gender].filter((match) => match.grid === grid).reduce((maxRound, match) => {
        return match.round > maxRound ? match.round : maxRound;
    }, 1);
...
function renderMatch(match) {
    ...
    const bracketRound = matchNode.querySelector(`[data-round="${match.round}"]`) || document.createElement("div");
...
function renderGrandFinal(match) {
    const grandFinalNode = match.gender === "boys"
...
function setBracketHeight(matches, querySelector, grid) {
    const rounds = matches.filter((match) => match.grid === grid && match.round === 1).length;
...
function renderPage() {
    ...
    renderRoundsNumbers(matches, "#girls-losers-bracket .bracket-headers", "girls", "lower");
