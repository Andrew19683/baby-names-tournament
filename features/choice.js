...
function getRandomMatch() {
    ...
    if (isSameDay(currentDate, new Date(localStorage.getItem("lastPlayedDate") ?? "null"))) {
        if (Number(localStorage.getItem("todayCount")) >= DAILY_LIMIT) {
            document
    ...
    const currentMatch = allMatches.find((match) => match.status === "currentMatch");
    if (currentMatch) {
        return currentMatch;
    }
    const possibleMatches = allMatches.filter((match) => match.status === "readyToPlay");
...
function renderMatch(match) {
    ...
    document.body.classList.add("gender-" + match.gender);
    const isNamesShuffled = Math.random() > 0.5 ? false : true;
    if (isNamesShuffled) {
        name1Node.textContent = match.player1.name;
        divName1Node.dataset.playerId = String(match.player1.id);
        name2Node.textContent = match.player2.name;
        divName2Node.dataset.playerId = String(match.player2.id);
    }
    else {
        name1Node.textContent = match.player2.name;
        divName1Node.dataset.playerId = String(match.player2.id);
        name2Node.textContent = match.player1.name;
        divName2Node.dataset.playerId = String(match.player1.id);
    }
    renderDescriptions();
...
function leftChoiceClick() {
    ...
    winnerId = Number(divName1Node.dataset.playerId);
    loserId = Number(divName2Node.dataset.playerId);
}
function rightChoiceClick() {
    divName2Node.classList.add("name-card--selected");
    divName1Node.classList.remove("name-card--selected");
    btnChooseNode.disabled = false;
    winnerId = Number(divName2Node.dataset.playerId);
    loserId = Number(divName1Node.dataset.playerId);
}
function setMatchResult() {
    ...
    // найдем текущий матч
    const matches = JSON.parse(localStorage.getItem("matches") ?? "null");
    const currentMatch = matches.boys.find((match) => match.status === "currentMatch") ||
        matches.girls.find((match) => match.status === "currentMatch");
    const result = resolveMatch(matches, currentMatch.id, currentMatch.gender, winnerId, loserId);
    ...
    if (currentMatch.id !== 1000) {
        ...
        popup.querySelector(".popup__gender-label").textContent =
            `Победитель среди ${currentMatch.gender === "boys" ? "мальчиков" : "девочек"}`;
    ...
    localStorage.setItem("todayCount", (parseInt(localStorage.getItem("todayCount") || "0") + 1).toString());
}
...
function checkForUnfinishedMatches() {
    const matches = JSON.parse(localStorage.getItem("matches") ?? "null");
    if ([...matches.boys, ...matches.girls].some((match) => match.status !== "finished")) {
        return true;
    }
    else {
        return false;
